...
def run_split(
    ...
    if train_mode == "segment":
        ...
        for no_entry, total, label, frame_no, total_frames in yolo.prepare_polygons():
            logger.debug(f"  [{no_entry}/{total}] {label}: frame {frame_no}/{total_frames}")
    else:
    ...
    for no_entry, total, label, split, frame_no, total_frames in gen:
        logger.debug(f"  [{no_entry}/{total}] {label} ({split}): frame {frame_no}/{total_frames}")
...
def _print_split_summary(label_dict, seed):
    """Log a per-label, per-split frame count table."""
    rows = []
