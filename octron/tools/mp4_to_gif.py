...
class DropArea(QFrame):
    ...
    def dragEnterEvent(self, event: QDragEnterEvent):
        ...
        if event.mimeData().hasUrls():
            ...
            if all(
                url.toLocalFile().lower().endswith((".mp4", ".mov", ".avi"))
                for url in urls
            ):
...
class MP4ToGifConverter(QMainWindow):
    ...
    def __init__(self):
        ...
        self.skip_spinner.setToolTip(
            "Skip N frames between each captured frame (0 = no skipping).\n"
        ...
        self.resize_spinner.setSpecialValueText(
            "No resize"
        )  # Show text when value is 0
        self.resize_spinner.setToolTip(
            "Resize output GIF to this maximum width (0 = original size).\n"
    ...
    def add_files(self, file_paths: list[str]):
        ...
        self.status_label.setText(
            f"Added {len(video_files)} files. Ready to convert."
        )
    ...
    def start_conversion(self):
        ...
        quality_settings = {
            ...
            2: {
                ...
                "diff_mode": "rectangle",
            },  # Low quality - use different dithering
        }
        ...
        for index, file_path in enumerate(self.files_to_process):
            ...
            # Update status
            self.status_label.setText(
                f"Converting {index + 1}/{total_files}: {input_path.name}"
            )
            ...
            try:
                ...
                # Update status
                self.status_label.setText(
                    f"Extracting frames from {input_path.name}..."
                )
                QApplication.processEvents()
                ...
                if skip_frames > 0:
                    ...
                    extract_cmd = [
                        ...
                        vf_filter,
                        "-vsync",
                        "0",  # Important: don't add duplicate frames
                ...
                logger.info(
                    f"Successfully extracted {num_frames} frames to {temp_dir}"
                )

                # Update status
                self.status_label.setText(
                    f"Creating GIF from {num_frames} frames..."
                )
                QApplication.processEvents()
                ...
                gif_cmd = [
                    ...
                    "-y",
                    "-framerate",
                    str(fps),  # Set input framerate
                    "-i",
                    f"{temp_dir}/frame_%04d.png",
                    "-i",
                    palette_path,
                    "-lavfi",
                    f"paletteuse=dither={quality_params['dither']}:bayer_scale={quality_params['bayer_scale']}:diff_mode={quality_params['diff_mode']}",
                    str(output_path),
                ]
                ...
                logger.exception(e)
                self.status_label.setText(
                    f"Error converting {input_path.name}: {str(e)}"
                )
