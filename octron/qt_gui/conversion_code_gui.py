...
content = re.sub(
    r"def setupUi\(self, octron_widgetui\)", "def setupUi(self)", content
)
...
content = re.sub(
    ...
    lambda m: (
        m.group(1)
        + re.sub(r"\bself\.(?!octron\.)", "self.octron.", m.group(2))
    ),
    content,
    flags=re.MULTILINE,
)
...
# Replace the video_file_drop_widget assignment for the predict drop
# widget line with the desired replacement.
content = re.sub(
