...
def make_linear_colormap(
    ...
    if reference_numbers is not None:
        reference_array = np.linspace(
            reference_numbers.min(), reference_numbers.max(), len(array)
        )
    else:
    ...
    if not categorical:
        ...
        if isinstance(cmap, list):
            ...
            if len(color_palette) < 100:
                logger.warning(
                    "Less than 100 distinct colors - is that what you want?"
                )
            ...
            raise NotImplementedError(
                f"Type {type(cmap)} for cmap argument not understood."
            )
        ...
        for el in array:
            if reference_array is None:
                index = int(
                    np.interp(
                        el,
                        [np.min(array), np.percentile(array, percentile)],
                        [0.0, 1.0],
                    )
                    * len(color_palette)
                )
                ...
                index = int(
                    (el / np.percentile(reference_array, percentile))
                    * len(color_palette)
                )
        ...
        for el in array:
            index = [
                no for no, category in enumerate(categories) if category == el
            ][0]
            color = color_palette[index]
            colors.append(color)
