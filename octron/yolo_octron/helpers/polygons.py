...
def find_objects_in_mask(
    ...
    all_properties = tuple(
        dict.fromkeys(("label", "area", "centroid") + tuple(properties))
    )

    # Collect extra property function names for output dict construction
    extra_prop_names = (
        tuple(fn.__name__ for fn in extra_properties)
        if extra_properties
        else ()
    )
    ...
    output_keys = [
        ...
        and (k.split("-")[0] in _user_requested or k in _user_requested)
    ]
...
def watershed_mask(
    ...
    assert not np.isnan(mask).any(), (
        "There are NaNs in input mask"
    )  # If this happens, it can be solved!
    assert set(np.unique(mask)) == set([1, 0]), (
        "Mask should be composed of 0s and 1s"
    )
    ...
    if plot:
        ...
        ax[1].set_title(
            f"Watershed, remaining masks: {len(masks)}\ndiam:{diam}"
        )
        plt.show()
...
def get_polygons(mask):
    ...
    assert not np.isnan(mask).any(), (
        "There are NaNs in input image"
    )  # If this happens, it can be solved!
    assert set(np.unique(mask)) == set([1, 0]), (
        "Image should be composed of 0s and 1s"
    )

    polygons = Mask(mask).polygons()
    polygons = (
        np.concatenate(
            merge_multi_segment([x.reshape(-1, 2) for x in polygons.points])
        )
...
def polygon_to_mask(
    ...
    if opening_radius > 0:
        ...
        if max(h0, w0) < model_imgsz:
            ...
            mask = cv2.resize(
                mask, (w1, h1), interpolation=cv2.INTER_NEAREST_EXACT
            )
