...
def load_boxmot_trackers(trackers_yaml_path):
    ...
    assert trackers_yaml_path.exists(), (
        f"Path {trackers_yaml_path} does not exist"
    )

    # Load the model YAML file and convert it to a dictionary
    with open(trackers_yaml_path) as file:
...
def resolve_tracker(tracker_name, trackers_dict):
    ...
    available = {
        tid: info
        for tid, info in trackers_dict.items()
        if info.get("available", False)
    }

    if not available:
        raise ValueError(
            "No trackers are marked as available in the tracker configuration."
        )
